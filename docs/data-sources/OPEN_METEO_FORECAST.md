...
- **Fields used:** Use one forecast request for a single explicit `WeatherLocation` with WGS84 `latitude`, `longitude`, `timezone` set to that location's IANA zone, `timeformat=iso8601`, canonical metric units (`temperature_unit=celsius`, `wind_speed_unit=kmh`, `precipitation_unit=mm`), `forecast_days=10` for the target daily horizon, and `forecast_hours=72` for the target rolling hourly horizon. The installed production request remains at 48 hours until the separately planned forecast-horizon data-contract slice changes and verifies it; this contract change is not an implementation claim.
  ...
  Hourly fields for the rolling 72-hour Home forecast and near-term precipitation:
  ...
  Daily fields for the ten-day Home forecast and sun/source blocks:
...
- **Error responses:** Open-Meteo forecast docs state that invalid URL parameters return HTTP 400 with a JSON object containing `error: true` and `reason`. The installed client classifies invalid response/request, network/offline failure, provider unavailable/5xx, HTTP/rate-limit where detectable, and unexpected body shape. Provider-specific error details do not cross into Composables.
...
- **Fixture/sample response location:** Implemented parser fixtures live under `core/src/test/resources/providers/openmeteo/`, covering normal Home forecast response, missing optional values, malformed envelope, invalid weather code, rate/error response body, and timezone-sensitive response.
