...
- **Purpose:** Active installed-app place-search provider for manual location selection through the production location-entry surface.
...
- **Error responses:** Open-Meteo geocoding docs state that invalid URL parameters return HTTP 400 with a JSON object containing `error: true` and `reason`. The installed geocoding client and repository classify empty results, invalid request/response, network/offline failure, provider unavailable/5xx, HTTP/rate-limit where detectable, and unexpected body shape. Provider-specific error bodies do not cross into Composables.
