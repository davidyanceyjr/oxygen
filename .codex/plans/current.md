...
Current phase: ready
Last result: Slice 17 review repair is implemented and verified. Stale cached Home refresh-failure copy no longer reuses terminal no-cache wording, and `CachedWeatherRepository` maps non-runtime storage read/write exceptions to `ForecastError.LocalCacheFailure` without catching `Error`. App default durable cache wiring, installed-app offline cache behavior, offline launch, saved-location persistence, unit preferences, alert/air-quality/radar cache, background work, release behavior, and active installed-app MET Norway fallback remain out of scope and unimplemented.
Blocker: none.
...
- implemented: Slice 17 review repair split stale refresh-failure copy into `HomeRefreshFailureMessage` so cached-stale Home state cannot show the terminal "No cached forecast is available yet" wording, and broadened `CachedWeatherRepository` storage failure handling from `RuntimeException` to `Exception` for cache write/readback and failed-refresh retention reads.
- verified: Review-repair focused checks passed: `. scripts/android-env.sh && ./gradlew :app:testDebugUnitTest --tests '*HomeForecastStateHolderTest'` and `. scripts/android-env.sh && ./gradlew :core:testDebugUnitTest --tests '*CachedWeatherRepositoryTest'`. Logs saved at `.codex/test-artifacts/2026-08-27-slice-17-review-repair/focused-home-state-tests.log` and `.codex/test-artifacts/2026-08-27-slice-17-review-repair/focused-cache-tests.log`.
- verified: Review-repair broad checks passed: `. scripts/android-env.sh && ./gradlew :app:compileDebugKotlin`, `. scripts/android-env.sh && ./gradlew :app:testDebugUnitTest :core:testDebugUnitTest`, `. scripts/android-env.sh && ./gradlew :app:assembleDebug`, and `git diff --check`. Logs saved at `.codex/test-artifacts/2026-08-27-slice-17-review-repair/broad-compile-debug-kotlin.log`, `.codex/test-artifacts/2026-08-27-slice-17-review-repair/broad-unit-tests.log`, `.codex/test-artifacts/2026-08-27-slice-17-review-repair/broad-assemble-debug.log`, and `.codex/test-artifacts/2026-08-27-slice-17-review-repair/git-diff-check.log`.
