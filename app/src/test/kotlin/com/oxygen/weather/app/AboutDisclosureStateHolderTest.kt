...
class AboutDisclosureStateHolderTest {
    ...
    fun `data sources disclosure separates active implemented and roadmap-only providers`() {
        ...
        assertTrue(text.contains("MET Norway forecast is implemented as a provider path"))
        assertTrue(text.contains("core fallback-selection capability"))
        assertTrue(text.contains("not wired as the active installed-app forecast fallback"))
        assertTrue(text.contains("cache persistence"))
        assertTrue(text.contains("foreground failed-refresh stale retention"))
        assertTrue(text.contains("Installed-app durable cache wiring"))
        assertTrue(text.contains("offline cache launch behavior are not implemented"))
        ...
        assertFalse(text.contains("saved locations are implemented"))
    ...
    fun `privacy disclosure contains active request facts and MET Norway capability facts`() {
        ...
        assertTrue(text.contains("does not use MET Norway as active installed-app forecast fallback"))
    }
